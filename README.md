...
ICAN 是一个面向真实产品演进的数字人 Agent 底层骨架（当前版本 v0.1.50）。第一版先打通浏览器、认证网关、LangGraph 编排、MCP、RAG、数字人 Provider 和可观测性，不绑定具体行业人格或业务工具。
...
本版本 Pi Agent 架构适配与 TTSA 修复验收记录见 [`tmp-docs/digital-human-agent-v0.1.50-pi-agent-adaptation-task-book-已完成.md`](tmp-docs/digital-human-agent-v0.1.50-pi-agent-adaptation-task-book-已完成.md)。
