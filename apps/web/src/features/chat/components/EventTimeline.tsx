import { BrainCircuit, Check, CircleAlert, CirclePlay, Clock3, Database, ListChecks, PauseCircle, Radio, ShieldCheck, Sparkles, Wrench } from 'lucide-react'
...
export function EventTimeline({ items, collapsed = false }: EventTimelineProps) {
  return (
        ...
        <ol className="timeline-list">
          {items.map((item) => {
            ...
            return <li key={item.id} className={`timeline-item timeline-item--${item.type}`}>
              ...
              <div className="timeline-copy"><div><strong>{item.title}</strong><time>{formatTime(item.createdAt)}</time></div>{item.detail && <pre>{item.detail}</pre>}</div>
            </li>
          })}
        </ol>
      )}
