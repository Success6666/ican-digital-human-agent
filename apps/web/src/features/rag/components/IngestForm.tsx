...
export function IngestForm({ isBusy, result, onSubmit }: IngestFormProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!sourceName.trim() || !collection.trim() || !content.trim() || isBusy) return
    await onSubmit({ sourceName: sourceName.trim(), collection: collection.trim(), content: content.trim() })
    setContent('')
  }
  ...
  return (
    <section className="work-panel" aria-labelledby="ingest-title">
      ...
      <form className="stack-form" onSubmit={submit}>
        ...
        <div className="form-actions"><small>{content.length.toLocaleString()} 字符</small><button className="primary-button" type="submit" disabled={isBusy || !content.trim()}>{isBusy ? <LoaderCircle size={15} className="spin" /> : <FilePlus2 size={15} />}{isBusy ? '处理中' : '开始入库'}</button></div>
      </form>
