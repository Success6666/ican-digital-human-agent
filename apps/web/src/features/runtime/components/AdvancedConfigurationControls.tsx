...
export function AdvancedConfigurationControls({ configuration, canManage, isLoading, isSaving, onSave }: Props) {
  ...
  const [embeddingProvider, setEmbeddingProvider] = useState('hash-local')
  const [embeddingBaseUrl, setEmbeddingBaseUrl] = useState('')
  const [embeddingApiKey, setEmbeddingApiKey] = useState('')
  const [embeddingModel, setEmbeddingModel] = useState('hash-256')
  const [embeddingDimensions, setEmbeddingDimensions] = useState('256')
  ...
  useEffect(() => {
    ...
    setEmbeddingEnabled(embedding.enabled !== false); setEmbeddingProvider(embedding.provider ?? 'hash-local'); setEmbeddingBaseUrl(String(embedding.baseUrl ?? '')); setEmbeddingModel(String(embedding.model ?? 'hash-256')); setEmbeddingDimensions(String(embedding.dimensions ?? 256)); setEmbeddingApiKey('')
  ...
  async function save() {
    ...
    try {
      ...
      if (dialog === 'llm') {
        ...
        if (embeddingProvider !== 'hash-local' && (!embeddingBaseUrl.trim() || (!configuration?.embedding?.configured && !embeddingApiKey.trim()) || !embeddingModel.trim())) throw new Error('远程 Embedding 必须填写 Base URL、API Key 和模型名')
  ...
  return <>
    <section className="configuration-summary" aria-labelledby="advanced-config-title">
      ...
      <div className="configuration-summary-grid">
        ...
        <ConfigCard icon={<Database size={17} />} title="Embedding" detail={configuration?.embedding?.detail ?? '本地特征哈希向量'} onClick={() => open('embedding')} disabled={!canManage || isLoading} />
    ...
    {dialog && <div className="configuration-dialog-backdrop" role="presentation" onMouseDown={() => !isSaving && setDialog(null)}><section className="configuration-dialog" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="configuration-dialog-body">
        ...
        {dialog === 'embedding' && <div className="configuration-mofa-form"><Switch label="启用 Embedding" checked={embeddingEnabled} onChange={setEmbeddingEnabled} /><Input label="Provider" value={embeddingProvider} onChange={setEmbeddingProvider} placeholder="hash-local 或 openai-compatible" /><Input label="Base URL" value={embeddingBaseUrl} onChange={setEmbeddingBaseUrl} /><Input label="API Key" value={embeddingApiKey} onChange={setEmbeddingApiKey} type="password" placeholder={configuration?.embedding?.configured ? '已配置，留空保持不变' : '远程服务 API Key'} /><Input label="模型名" value={embeddingModel} onChange={setEmbeddingModel} /><Input label="向量维度" value={embeddingDimensions} onChange={setEmbeddingDimensions} type="number" /></div>}
