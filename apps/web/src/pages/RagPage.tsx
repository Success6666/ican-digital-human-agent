...
export function RagPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <RagOverview health={rag.health} activeTab={activeTab} onTabChange={setActiveTab} onIngest={() => setActiveTab('documents')} onFocusSearch={() => setActiveTab('search')} />
      {activeTab === 'documents' && <div className="rag-grid rag-tab-panel"><IngestForm isBusy={rag.isBusy} result={rag.ingestResult} onSubmit={rag.ingest} /></div>}
      {activeTab === 'search' && <div className="rag-grid rag-tab-panel"><SearchForm isBusy={rag.isBusy} hits={rag.hits} onSubmit={rag.search} /></div>}
    </div>
  )
}
