...
import { AdvancedConfigurationControls } from '../features/runtime/components/AdvancedConfigurationControls'
...
export function SettingsPage({ avatar, canManage }: SettingsPageProps) {
  ...
  const agentStatus = avatar.isLoading ? 'pending' : avatar.providers.length ? 'online' : 'offline'
  const agentDetail = avatar.isLoading ? '读取 Provider 状态' : avatar.providers.length ? 'Provider 注册表已就绪' : '无法读取 Provider 状态'
  return (
    <div className="page-stack">
      ...
      <AdvancedConfigurationControls configuration={configuration.configuration} canManage={canManage} isLoading={configuration.isLoading} isSaving={configuration.isSaving} onSave={configuration.save} />
